...
$countProps = 1;
($arParams["ID_PROPERTIES"])? $arSelectProps = $arParams["ID_PROPERTIES"] : $arSelectProps = array(396, 395, 431);
if (!CSite::InDir('/catalog/')){
    $linkCatalog = "/catalog/_diski";
}else{
    $linkCatalog = "";
}
?>
<div class="filter-item__form">
		<form name="disk_form" action="<?=$linkCatalog.$arResult['FORM_ACTION']?>" method="get" class="smartfilter2">
			<?foreach($arResult["HIDDEN"] as $arItem):?>
			<input type="hidden" name="<?echo $arItem['CONTROL_NAME']?>" value="<?echo $arItem['HTML_VALUE']?>" />
			<?endforeach;?>

<?/*только указанные свойства*/
foreach ($arSelectProps as $key){
						...
						<div class="bx-filter-block" data-role="bx_filter_block">
							...
							$arCur = current($arItem["VALUES"]);    
								...
								<div class="bx-filter-select-container /*bx_filter_select_container*/ filter-item__sizes-item">
									...
									<div class="bx_filter_select_block" data-type="disk" onclick="smartFilter.showDropDownPopup2(this, '<?=CUtil::JSEscape($key)?>')">
...
<?}
							...
							<div class="bx-filter-block" data-role="bx_filter_block">
								<div class="bx-filter-parameters-box-container">
										...
										<div class="form-slider__input-wrapper">
                                                                                    ...
                                                                                    if ($_GET["catalogFilter2_P3_MIN"])
                                                                                        $minPriceFilter = $_GET["catalogFilter2_P3_MIN"];
                                                                                    ?>
											...
											/>
                                                                                ...
                                                                                <?$maxPriceFilter = (int)$arItem["VALUES"]["MAX"]["VALUE"];
                                                                                    if ($_GET["catalogFilter2_P3_MAX"])
                                                                                        $maxPriceFilter = $_GET["catalogFilter2_P3_MAX"];
                                                                                    ?>                                                                                
										<div class="form-slider__input-wrapper">
											<span class='rubl'>&#8381;</span><input
				...
				?>
					...
					<div class="form-group form-group--btn d-flex justify-content-center">
						<input class="btn btn-scarlet" type="submit" id="set_filter_2" name="set_filter" value="<?=GetMessage("CT_BCSF_SET_FILTER")?>" />
						<input type="submit" id="del_filter_2" name="del_filter" value="<?=GetMessage("CT_BCSF_DEL_FILTER")?>" style='display: none;'/>
