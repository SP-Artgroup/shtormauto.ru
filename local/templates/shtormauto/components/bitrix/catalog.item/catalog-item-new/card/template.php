...
<div class="product-item__wrapper">
	...
	<div class="item-label">
		<div class="labels-container">
			<?if($arResult['ITEM']['PROPERTIES']['TIRE_LONG_LIVER']['VALUE_XML_ID'] == 'Y'):?>
				<img class="label" src="/local/templates/shtormauto/images/label/shina_dolgozhitel.png" alt="Шина долгожитель">
			<?endif?>
			<?if($arParams['UF_BRIDGESTONE'] == 'Y'):?>
				<img class="label" src="/local/templates/shtormauto/images/label/5_year_warranty_mini.png" alt="Двойная гарантия Bridgestone">
			<?endif?>
			<?if($arResult['ITEM']['PROPERTIES']['BRAND_N1_IN_RUSSIA']['VALUE_XML_ID'] == 'Y'):?>
    ...
    <div class="product-item__specification">
        <?foreach ($item["DISPLAY_PROPERTIES"] as $arPropsItem){?>
            <?if (!empty($arPropsItem["~VALUE"]) && $arPropsItem["~VALUE"] != '*'): ?>
        <?= $arPropsItem["NAME"] ?>: <?= $arPropsItem["~VALUE"] ?> <br>
            <?endif ?>
        <?}?>
    </div>
    <? if (($actualItem['CAN_BUY'] && !empty($itemIds['PRICE']) && $price["PRICE"] > 0) || $arParams["SHOW_PRICE"] == "Y"):?>
        <div class="product-item__price" data-entity="price-block">
            ...
            <?}?>
            <span class="main-price" id="<?= $itemIds['PRICE'] ?>">
                <?= ($price["PRICE_WITH_EXCHANGE"]) ? $price["PRICE_WITH_EXCHANGE"] : $price["PRICE"]; echo Loc::getMessage('RUB');?>
            </span>
            <? if ($price["PRICE_WITH_EXCHANGE"]): ?>
                <span class="price-decript"><?=Loc::getMessage('HANDING_OVER_OLD');?></span>
                <span class="price-without-descount"><?=$price["BASE_PRICE"] . Loc::getMessage('RUB');?></span>
            <? endif; ?>
        </div>
    <? endif ?>
    ...
    <div class="product-item__buy">
        ...
        <? if (!$haveOffers) {?>
            <div>
                ...
                <? endif ?> 
                <? if(!empty($arParams['CITY_AMOUNTS']) && !$actualItem['CAN_BUY']):?> 
                    <div class="btn-not-avail scarlet"><strong>Доставка от 3-х дней</strong></div>
                <? endif ?>
                <? if (!empty($arParams['CITY_AMOUNTS']) && !$actualItem['CAN_BUY']): ?>
                ...
                <?if ($actualItem['CAN_BUY']) { ?>
                    <div class="browse_but2">
                        <button
                            ...
                            data-max-amount="<?=$arParams['CITY_AMOUNTS'][$arParams["CURRENT_CITY"]]?>"
                            >
                            <span><?=
                                (
