...
<? if (isset($arResult["REQUEST"]["ORIGINAL_QUERY"])): ?>
	...
	<div class="search-language-guess">
		<?= GetMessage("CT_BSP_KEYBOARD_WARNING", array(
...
<? endif ?>
	...
	<?elseif(count($arResult["SEARCH"])>0):?>
		...
		<?
			...
			//$cities_list   = SP\City::getAllCities();
		?>
		...
		<?foreach($arResult["SEARCH"] as $arItem):?>
			...
			$isProduct    = $arItem['PARAM1'] == 'catalog' && intval($itemId);
			/*$hasAmount    = isset($cityAmounts[$itemId]);
			$AmountByCity = $hasAmount ? $cityAmounts[$itemId] : null;

			if ($isProduct && !$hasAmount) {
				continue;
			}*/
			...
			<div class="search-item">
				<div class="search-item-left">
					<? if($arItem['PARAM1'] == 'catalog' && intval($itemId)) { ?>
						<? if (isset($arResult['PRODUCTS'][$arItem['ITEM_ID']])) { ?>
							<?
								$arProduct	= $arResult['PRODUCTS'][$itemId];
							?>
							<? if(!empty($arProduct['PREVIEW_PICTURE']['SRC'])):?>
								<img src="<?=$arProduct['PREVIEW_PICTURE']['SRC']?>" alt="<?=$arProduct['NAME']?>" />
							<? endif;?>
						<? } ?>
					<? } ?>
				</div>
				<div class="search-item-name <?=(empty($arProduct['PREVIEW_PICTURE']['SRC']))?'no-image':''?>">
					<? if ($isProduct && $arResult['PRODUCTS'][$arItem['ITEM_ID']]['CATALOG_QUANTITY'] < 1) { ?>
						<h4><?= $arItem["TITLE_FORMATED"] ?></h4>
					<? } else { ?>
						<h4>
							<a href="<?= $arItem["URL"] ?>"><?= $arItem["TITLE_FORMATED"] ?></a>
						</h4>
					<? } ?>
					<?if(!empty($arResult["PRODUCTS"][$arItem['ITEM_ID']]["PROPERTY_ICON_VALUE"])):?>
						<div class="block_tempary_icons d-flex flex-wrap justify-content-between alignt-items-center">
							<? foreach ($arResult["PRODUCTS"][$arItem['ITEM_ID']]["PROPERTY_ICON_VALUE"] as $key => $value) {?>
				...
				<? if ($arItem['PARAM1'] == 'catalog'): ?>
					...
					<div class="search_city_link">
						<? /*if (!$AmountByCity[$currentCityId]) { ?>
							<strong>В выбранном городе данный товар отсутствует!</strong>
							<br>
						<? } ?>
						<? if (isset($cityAmounts[$arItem['ITEM_ID']])): ?>
							<?
							$cityIds = array_keys($cityAmounts[$arItem['ITEM_ID']]);
							$city_link = [];

							foreach ($cityIds as $cityId) {
								$cityName = $cities_list[$cityId]['NAME'];
								$city_link[] = '<a href="' . $arItem['URL'].'&chcity='. $cityId . '">' . $cityName . '</a>';
							}
							...
							<? } ?>
						<? endif */?>
						<? if($arResult['PRODUCTS'][$arItem['ITEM_ID']]['CATALOG_QUANTITY'] > 0):?>
							В наличии
						<? else:?>
				...
				<div class="search-item-price">
					<? if(isset($arProduct['PRICE'])) { ?>
						<strong>от <?=CurrencyFormat($arProduct['PRICE'], 'RUB')?></strong><br/>
					<? } ?>
				</div>
