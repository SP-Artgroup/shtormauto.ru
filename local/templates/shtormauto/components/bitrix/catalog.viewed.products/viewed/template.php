...
//dump($arResult['ITEMS']['70583']);
?>
<?
if ( !empty($arResult['ITEMS'])) 
{
?>
	<!-- /************************НОВОЕ (Структура товара полностью таже, добавлен новый клас дива)*****************************/ -->
<?//	<div style="clear: both;"></div>?>
	<div class="catalog-section catalog_section_new hidmobile viewed_products">
	<div class="catalog-new_head">Вы смотрели</div>
		<? foreach($arResult['ITEMS'] as $arItem) { ?>
			<div class="catalog-item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
				...
				<?else:?>
					<div class="img-wrap">
					<a href="<?=$arItem["DETAIL_PAGE_URL"]?>">
						<img border="0" src="<?=$templateFolder?>/images/no-photo.png" alt="<?=$arItem["NAME"]?>" title="<?=$arItem["NAME"]?>" />
					</a>
					<br />
					</div>
				<?endif?>
				<?/*
	<div style="_background: url('<?=$arItem['DETAIL_PICTURE']['SRC']?>') center center no-repeat;" class="img-wrap">
					<a href="<?=$arItem['DETAIL_PAGE_URL']?>"><img src="<?=$arItem['DETAIL_PICTURE']['SRC']?>" alt="<?=$arItem['NAME']?>" title="<?=$arItem['NAME']?>" border="0" height="120px" width="140px"></a><br>
				</div>
	*/?>
				<a class="product_viewed_name" href="<?=$arItem['DETAIL_PAGE_URL']?>"><?=$arItem['NAME']?></a><br>
					<span class="catalog-price"><?=$arItem['MIN_PRICE']['PRINT_VALUE']?></span>

				<div class="browse_but2">
					<button type="button" class="button pokupka action_ajax" href="/local/ajax/action.php?action=add_to_basket&product_id=<?=$arItem['ID']?>" onclick="javascript:send_in_cart('<?=$this->GetEditAreaId($arItem['ID']);?>')" ><span>Купить</span></button>
