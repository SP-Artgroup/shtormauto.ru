...
<div class="bx-auth-serv-icons">
	...
	?>
	<a title="<?=htmlspecialcharsbx($service["NAME"])?>" href="javascript:void(0)" <?=$onClickEvent?>  class="btn btn-denim-blue" id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>">
		Войти с помощью <?=htmlspecialcharsbx($service["NAME"])?>
	</a>
	<?endforeach?>
</div>
<?endif;?>
	...
	<form method="post" name="bx_auth_services<?=$arParams["SUFFIX"]?>" target="_top" action="<?=$arParams["AUTH_URL"]?>">
		...
		<?if($arParams["~FOR_SPLIT"] != 'Y'):?>
			<div class="bx-auth-services">
				<?foreach($arAuthServices as $service):?>
					<div><a href="javascript:void(0)" onclick="BxShowAuthService('<?=$service["ID"]?>', '<?=$arParams["SUFFIX"]?>')" class="btn btn-denim-blue" id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>">Войти с помощью <?=htmlspecialcharsbx($service["NAME"])?></a></div>
				<?endforeach?>
			</div>
		<?endif;?>
