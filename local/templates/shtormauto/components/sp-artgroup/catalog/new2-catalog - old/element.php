...
?>
		...
		if (!$elementId) {
			...
			$elementCode = $GLOBALS['REDIRECT_ELEMENT_CODE'];

			$redirectUrl = CIBlockElement::getList(
				[],
				[
		...
		if ($elementId > 0)
		{
			...
			<?// arSimilarProductsFilter задаётся в
			// bitrix:catalog.element/catalog-element/component_epilog.php

			global $arSimilarProductsFilter;
			if (!empty($arSimilarProductsFilter['PROPERTY_MODEL_VALUE'])) {

				$arSimilarProductsFilter['!ID'] = $elementId;

				$availableProductsFilter = SP\Component::getAvailableProductsFilter();
				$arSimilarProductsFilter = array_merge($arSimilarProductsFilter, $availableProductsFilter);

				$APPLICATION->IncludeComponent(
					"bitrix:catalog.section",
					"catalog-section-incard",
					array(
						"IBLOCK_TYPE"         => $arParams['IBLOCK_TYPE'],
						"IBLOCK_ID"           => $arParams['IBLOCK_ID'],
						"SECTION_ID"          => "",
						"SECTION_CODE"        => "",
						"SECTION_USER_FIELDS" => [],
						"ELEMENT_SORT_FIELD"  => "CATALOG_PRICE_" . Shtormauto::getInstance()->getCurrentCityPriceId(),
						"ELEMENT_SORT_ORDER"  => "asc",
						"ELEMENT_SORT_FIELD2" => "id",
						"ELEMENT_SORT_ORDER2" => "desc",
						"FILTER_NAME"         => "arSimilarProductsFilter",
						...
						"CACHE_FILTER" => "Y",
						"PRICE_CODE" => array(
							0 => Shtormauto::getInstance()->getCurrentCityPriceName(),
						),
						"USE_PRICE_COUNT" => "N",
						"SHOW_PRICE_COUNT" => "1",
						"PRICE_VAT_INCLUDE" => "Y",
						...
						"DISABLE_INIT_JS_IN_COMPONENT" => "N",
                                                "TITLE_BLOCK" => "Другие размеры этой модели"                                                
					),
					$component
				);
