...
<div class="grey-block filter">
    <div class="grey-block-title"><?=$arParams["FILTER_DISPLAY_NAME"]?></div>
        		...
        		<?foreach($arResult["FILTER_VALUES"] as $code=>$arItem):?>
					...
					<select id="ul_<?echo $arItem["CODE"]?>" name="FILTER[<?=$arItem["CODE"]?>]" class="propselect">
        					...
        					<?foreach($arItem["VALUES"] as $val => $value):?>
			                                <option class="prop" value="<?=$value["VALUE"];?>" name="<?echo $ar["CONTROL_NAME"]?>" id="<?echo $ar["CONTROL_ID"]?>" <?echo $value["SELECTED"]?'selected="selected"':''?>>
			                                    <?=$value["VALUE"];?>
			                                </option>
        					<?endforeach;?>
        				</select>
