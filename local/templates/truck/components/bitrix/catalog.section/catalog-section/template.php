...
$langPrefix = 'CT_BCS_CATALOG_';

$productsInSection = Loc::getMessage(
    $langPrefix . 'PRODUCTS_IN_SECTION',
    [
        '#quantity#'   => '<i>' . $arResult['data']['prod_in_sect'] . '</i>',
        '#declension#' => $arResult['data']['prod_in_sect_decl'],
    ]
);
...
<div class="catalog-section fix-gutter" data-entity="<?=$containerName?>">
    <?
...
<div class="filter-setting">
    ...
    <p class="quantity"><?= $productsInSection ?></p>
        ...
        <div class="select-container">
            <select class="selectcustom js-showby-select">
                <?php foreach ($arParams['SHOWBY_PARAMS'] as $showBy): ?>
                    <option
                        value="<?= $showBy ?>"
                        <?php if ($showBy === $arParams['PAGE_ELEMENT_COUNT']): ?>
                            selected
                        <?php endif ?>

                    ><?= Loc::getMessage($langPrefix . 'SHOW_BY', [
                        '#quantity#' => $showBy,
                    ]) ?></option>
                <?php endforeach ?>
            </select>
...
<div class="not-find-container">
    ...
    <div class="not-find">
        <p>Оставьте заявку и наш специалист поможет Вам в выборе и поиске подходящего товара для Вашего автомобиля.</p>
        <button class="btn1 btn-black js-request">Оставить заявку</button>
    </div>
</div>
