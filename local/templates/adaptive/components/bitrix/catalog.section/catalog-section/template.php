...
$catalogItemTemplate = 'catalog-item';
...
echo '<pre style="display:none">'; print_r($APPLICATION->GetPageProperty('additional_text')); echo '</pre>';
echo '<pre style="display:none">'; print_r($arResult['IPROPERTY_VALUES']); echo '</pre>';
?>
<div class="catalog-section bx-<?=$arParams['TEMPLATE_THEME']?>" data-entity="<?=$containerName?>">
	...
	<?php if ($showSort): ?>
		<div class="row">
			<div class="col-xs-12">
				<div class="sort-block pull-right" onclick="window.location='<?= $sortUrl ?>'">
					<span><?= Loc::getMessage('CT_BCS_CATALOG_SORT_TITLE') ?></span>
					<span class="sort <?= $sortClass ?>" title="<?= $sortMsg[$sortOrder] ?>"></span>
				</div>
			</div>
		</div>
	<?php endif ?>

	<div class="row custom-gutter-row-10">
	<?
	if ($hasItems) {
			...
			<!-- <div class="row <?=$rowData['CLASS']?>" data-entity="items-row"> -->
				...
				switch ($rowData['VARIANT'])
				{
					...
					case 2:
						...
						{
							?>
							<div class="col-xs-6 col-sm-4 product-item-big-card custom-gutter-col-10">
								<div class="row">
									<div class="col-md-12">
										<?
										$APPLICATION->IncludeComponent(
