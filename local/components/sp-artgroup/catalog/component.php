...
if($arParams["SEF_MODE"] == "Y")
{
	...
	if (!empty($arVariables['ELEMENT_CODE'])) {
		$GLOBALS['REDIRECT_ELEMENT_CODE'] = $arVariables['ELEMENT_CODE'];
	}

	if($b404 && CModule::IncludeModule('iblock'))
	{
		$redirectUrl = '';
		$sectionPath = $arVariables['SECTION_CODE_PATH'] ?? null;

		if ($sectionPath) {

			if (strpos($sectionPath, '/') !== false) {
				$componentPage = 'element';
				$elementCode   = explode('/', $sectionPath)[1];
			} else {
				$componentPage = 'section';
				$sectionCode   = $sectionPath;
			}
		}

		if ($componentPage === 'section') {
			$redirectUrl = CIBlockSection::getList(
				[],
				[
