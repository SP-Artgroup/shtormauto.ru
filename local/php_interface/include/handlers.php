...
$handlers = [
    ...
    //удаление скидочного купона после оформления заказа
    ['sale', 'OnSaleOrderSaved', ['ShtormautoEvents', 'OnSaleOrderSavedHandler']],
];
...
class ShtormautoEvents
{
	...
	public function OnSaleOrderSavedHandler(Bitrix\Main\Event $event) : void {
        ...
        while ($coupon = $couponList->fetch())
        {
            ...
            if (strpos($coupon["COUPON"], "$idUserBasket-") == 0) {
                $couponsData = Sale\DiscountCouponsManager::getData($coupon["COUPON"]);
