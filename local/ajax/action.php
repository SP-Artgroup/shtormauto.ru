...
class ShtormautoAction
{
    ...
    public function BasketChangeQuantity()
    {
        ...
        if ($basketItem) {
            ...
            if (!$this->checkQuantity($productId, $currentCityId, $quantity, false)) {
    ...
    public function AddToBasket()
    {
        ...
        if (!$this->checkQuantity($productId, $currentCityId, $quantity)) {
    ...
    private function checkQuantity(int $productId, int $currentCityId, int $quantity, bool $isAddMode = true)
    {
        $cityShopIds = SPShop::getCityShops($currentCityId);
        $arCityQuantity = SPShop::getProductAmount($productId, $cityShopIds);

        $cityQuantity = 0;        
        foreach ($arCityQuantity[$productId] as $key => $value) {
            $cityQuantity = $cityQuantity + $value;
        }
        ...
        if ($quantity > ($cityQuantity - $basketQuantity)) {
            return false;
        }
